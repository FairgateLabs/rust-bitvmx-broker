...
- **Services** (`new_services`): serves several components inside one process. It takes a routing table naming who may talk to whom, and `send_service` queues a message for another component on the same broker, which `tick` delivers through a local channel.
...
- **Both `send_peer` and `send_service` wait for a `tick` to deliver.** They enqueue in the caller's shared storage and participate in its global transaction. Commit before ticking: `tick` must not deliver work you might roll back. Direct channel sends still reach the destination broker storage immediately.
