...
#[derive(Debug)]
pub struct BrokerClientAsync {
    address: SocketAddr,
    client: Arc<Mutex<Option<BrokerApiClient>>>,
    cert: Cert,
...
impl Clone for BrokerClientAsync {
    fn clone(&self) -> Self {
        self.try_clone().expect("failed to clone BrokerClientAsync")
    }
}
...
impl BrokerClientAsync {
    ...
    async fn connect(&self) -> Result<(), BrokerError> {
        ...
        let client = BrokerApiClient::new(client::Config::default(), transport).spawn();
        let mut locked = self.client.lock().await;
        *locked = Some(client);
        Ok(())
    }

    async fn get_or_connect(&self) -> Result<BrokerApiClient, BrokerError> {
        {
            let mut locked = self.client.lock().await;
