...
fn prepare_server(
    ...
    let local = LocalChannel::new("1".to_string(), storage);
    (server, local)
}
...
fn prepare_client(
    ...
    .unwrap();
    let user = DualChannel::new(&server_config, client_cert, allow_list).unwrap();
    user
}
...
fn create_allow_list(pubk_hashes: Vec<String>) -> Arc<Mutex<AllowList>> {
...
fn test_channel() {
    ...
    let allow_list =
        create_allow_list(vec![server.get_pkh(), client1.get_pkh(), client2.get_pkh()]);
    ...
    user1
        .send(Some(client2.get_pkh()), "Hello!".to_string())
        .unwrap();
    let msg = user2.recv().unwrap().unwrap();
    assert_eq!(msg.0, "Hello!");
    assert_eq!(msg.1, client1.get_pkh());
    broker_server.close();
    cleanup_storage(port);
}
...
fn test_ack() {
    ...
    myclient
        .send_msg(client1.get_pkh(), client2.get_pkh(), "Hello!".to_string())
        .unwrap();
    let msg = myclient.get_msg(client2.get_pkh()).unwrap().unwrap();
    assert_eq!(msg.msg, "Hello!");
    let msg_dup = myclient.get_msg(client2.get_pkh()).unwrap().unwrap();
    assert_eq!(msg.uid, msg_dup.uid);
    assert!(myclient.ack(client2.get_pkh(), msg.uid).unwrap());
    println!("{:?}", myclient.get_msg(client2.get_pkh()).unwrap());
    assert!(myclient.get_msg(client2.get_pkh()).unwrap().is_none());
    broker_server.close();
    cleanup_storage(port);
}
...
fn test_reconnect() {
    ...
    let msg = myclient.get_msg(client2.get_pkh()).unwrap().unwrap();
    assert_eq!(msg.msg, "Hello!");
    myclient.ack(client2.get_pkh(), msg.uid).unwrap();
    assert!(myclient.get_msg(client2.get_pkh()).unwrap().is_none());
    broker_server.close();

    let (mut broker_server, _) = prepare_server(port, &server.privk, allow_list.clone());
    myclient
        .send_msg(client1.get_pkh(), client2.get_pkh(), "World!".to_string())
        .unwrap();
    let msg = myclient.get_msg(client2.get_pkh()).unwrap().unwrap();
...
fn test_stress_channel() {
    ...
    for i in 0..1000 {
        println!("Sending: {}", i);
        let send_ok = user1.send(Some(client2.get_pkh()), "Hello!".to_string());
        if send_ok.is_err() {
...
fn test_dinamic_allow_list() {
    ...
    cleanup_storage(port);
    let (server, client1, client2) = get_keys();
    let allow_list = create_allow_list(vec![server.get_pkh(), client1.get_pkh()]);
    let (mut broker_server, _) = prepare_server(port, &server.privk, allow_list.clone());
    let user1 = prepare_client(port, &server.get_pkh(), &client1.privk, allow_list.clone());
    let user2 = prepare_client(port, &server.get_pkh(), &client2.privk, allow_list.clone());
